...
stdin, stdout, stderr = ssh.exec_command(f"echo REDACTED_PASSWORD | sudo -S {cmd}", timeout=300)
...
if exit_status == 0:
    ...
    out, err = run("echo REDACTED_PASSWORD | sudo -S docker compose -f /vol3/1000/docker/zscx/docker/docker-compose.yml up -d 2>&1")
    print(out.strip())

    out, err = run("echo REDACTED_PASSWORD | sudo -S docker ps --filter name=zscx --format '{{.Names}} {{.Status}}'")
    print("\n=== Containers ===")
    print(out.strip())
