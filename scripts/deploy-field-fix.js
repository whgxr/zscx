/** 临时：部署插入字段修复（组件+2 page+header）。用毕删除。 */
...
async function run() {
  ...
  const df = ['FROM zscx-web:local', 'USER root', 'COPY . .',
  ...
  console.log('2. 构建...')
  await sshExec(conn, `cd ${remoteWeb} && rm -f /tmp/f2.log && { docker build --progress=plain -f Dockerfile.incremental -t zscx-web:new . ; echo "EXIT=$?"; } > /tmp/f2.log 2>&1`, 1500000)
  const ex = (await sshExec(conn, "grep -o 'EXIT=[0-9]*' /tmp/f2.log | tail -1", 30000)).out || ''
  console.log('退出码:', ex)
