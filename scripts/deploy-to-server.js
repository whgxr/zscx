...
async function main() {
  ...
  try {
    ...
    // 5. 解压（先清空目录，避免残留已删除的旧文件；保留 docker-nas/.env 服务器配置）
    console.log('\n📂 清空旧代码并解压到目标目录...')
    await execRemote(conn, `cd ${CONFIG.remotePath} && cp docker-nas/.env /tmp/zscx-env-backup 2>/dev/null; ls -A | grep -v zscx-deploy.tar.gz | xargs -r rm -rf && tar -xzf zscx-deploy.tar.gz --strip-components=1 && rm zscx-deploy.tar.gz; [ -f /tmp/zscx-env-backup ] && cp /tmp/zscx-env-backup docker-nas/.env && rm /tmp/zscx-env-backup; true`)

    // 6. 构建 Docker 镜像
    console.log('\n🐳 构建 Docker 镜像...')
    await execRemote(conn, `cd ${CONFIG.remotePath}/web && docker build -t zscx-web:local .`)

    // 7. 创建 docker-compose override 使用本地镜像
    console.log('\n⚙️  配置 Docker Compose...')
    await execRemote(conn, `cat > ${CONFIG.remotePath}/docker-nas/docker-compose.override.yml << 'EOF'
services:
  web:
...
EOF`)
    ...
    // 8. 启动服务
    console.log('\n🚀 启动 Docker 服务...')
    await execRemote(conn, `cd ${CONFIG.remotePath}/docker-nas && docker compose down 2>/dev/null; docker compose up -d`)

    // 9. 检查状态
    console.log('\n📊 服务状态:')
    await execRemote(conn, `cd ${CONFIG.remotePath}/docker-nas && docker compose ps`)
